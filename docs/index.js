  ...
  function getIframe() {
    if (!document.iframe) {  
      ...
      if (iframe.sandbox.supports && iframe.sandbox.supports('allow-downloads')) iframe.sandbox.add('allow-downloads');
  ...
  async function renderContent() {
    ...
    let overwriteSelf = isWatch && renderer && !renderer.script.endsWith(".html");
...
async function recordToHistory(durl) {
  ...
  openRequest.onsuccess = () => {
    ...
    let transaction = db.transaction("urls", "readwrite");

    let history = transaction.objectStore("urls");
