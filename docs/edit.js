...
function fetchCodepen(url) {
  ...
  Promise.all([
    ...
    let durl_raw = `data:text/html;charset=utf-8,${encodeURIComponent(string)}`;

    let durl = new bitty.DataURL(durl_raw);
    durl = await durl.compress(bitty.GZIP_MARKER);

    let ratio = durl.href.length / durl_raw.length;
    console.log(`Compressed from ${durl_raw.length} to ${durl.href.length} bytes (${Math.round(ratio * 100)}%)`);
