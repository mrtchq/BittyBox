...
export const BittyEditor: React.FC<BittyEditorProps> = ({
  ...
  const [editorViewMode, setEditorViewMode] = useState<'split' | 'code'>('split');
  ...
  let previewHtml = '';
  if (!content.trim()) {
    previewHtml = `<!DOCTYPE html><html><body style="margin:0;display:flex;height:100vh;align-items:center;justify-content:center;background:#000000;color:#ffffff;font-family:sans-serif;"><p style="font-size:12px;opacity:0.8;font-family:monospace;color:#ffffff;">[ Live Preview Initialized &bull; Waiting for input ]</p></body></html>`;
  } else {
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
        ...
        {editorViewMode !== 'code' && (
          <div className={`bento-card live-preview-container ${editorViewMode === 'split' ? 'md:col-span-6 lg:col-span-6' : 'md:col-span-12'} p-5 flex flex-col min-h-[500px]`}>
            ...
            <div className="flex-1 w-full rounded overflow-hidden border border-cyan-500/30 bg-[#000000] relative shadow-inner min-h-[380px] flex items-center justify-center p-1">
              <div 
      ...
      {inlinePreviewActive && (
        <div className="flex flex-col gap-4">
          ...
          <div className="rounded-xl overflow-hidden border border-cyan-500/30 bg-[#000000] shadow-[0_0_30px_rgba(0,242,255,0.15)] min-h-[60vh] flex items-center justify-center p-2">
            <div
              className={`h-full transition-all duration-300 relative ${
                previewDevice === 'mobile'
                  ? 'w-[375px] max-w-full rounded-2xl border-2 border-cyan-500/40 shadow-[0_0_20px_rgba(0,242,255,0.2)] overflow-hidden'
            ...
            >
              <iframe
                ...
                srcDoc={previewHtml}
                title="Bitty Box Live Preview"
                className="w-full h-[60vh] border-0 absolute inset-0 bg-[#000000]"
                sandbox="allow-scripts allow-forms allow-same-origin allow-popups allow-modals allow-downloads"
              />
            </div>
          </div>
