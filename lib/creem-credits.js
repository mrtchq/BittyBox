...
let cachedBaseUrl = process.env.CREEM_API_URL || "https://test-api.creem.io/v1";
...
function loadCreemConfig() {
  ...
  if (!cachedApiKey) {
    throw new Error("CREEM_API_KEY is required for Creem credit operations");
  }
 ...
 */
export async function getOrCreateCustomerCreditAccount(customerId) {
  if (!customerId) throw new Error("Creem Customer ID required");

  // 1. Check existing accounts list
  try {
    const listRes = await creemRequest("/customer-credits/accounts");
    const accounts = listRes?.data || [];
    const found = accounts.find(a => a.customer_id === customerId);
    if (found) {
      return found;
    }
  } catch (err) {
...
export async function ensureUserCreemCreditAccount(user) {
  ...
  try {
    ...
    if (!creditAccountId && customerId) {
      const acc = await getOrCreateCustomerCreditAccount(customerId);
