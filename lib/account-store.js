...
export async function registerUser(email, displayName = "", password = "") {
    ...
    transactions: [
      {
        ...
        type: "grant",
        amount: 100,
        description: "Welcome Bonus Credits (Creem.io Customer Credits)",
  ...
  // Create Creem Customer & Credit Account and issue starter credits
  try {
    const creemInfo = await ensureUserCreemCreditAccount(newUser);
    if (creemInfo && creemInfo.creditAccountId) {
      await issueCreemCredits(creemInfo.creditAccountId, 100, "Welcome Starter Bonus Credits", `ik_welcome_${newUser.id}`);
...
export async function purchaseCredits(userId, packageId, amount, costCents = 0) {
  ...
  user.transactions.unshift({
    ...
    description: `Purchased ${creditAmount} Credits (Creem.io Customer Credits)`,
    createdAt: new Date().toISOString()
  });
...
export async function deductCredits(userId, amount = 1, category = "human", reference = "Usage deduction") {
  ...
  if ((user.credits || 0) < deductNum) {
    return false;
  }

  // Decrement on Creem.io Customer Credits API
  try {
    const creemInfo = await ensureUserCreemCreditAccount(user);
    if (creemInfo && creemInfo.creditAccountId) {
    ...
    console.error(`[account-store] Creem debit failed for ${user.id}:`, err.message);
    user.credits = Math.max(0, (user.credits || 0) - deductNum);
  }
