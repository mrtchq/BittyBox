...
const ROADMAP_LOCKS = [
  ...
  },
  {
    ...
    reason: '1 view, then gone',
    desc: 'Destroys the ciphertext and in-memory decryption context after the first opening.',
    whyNotToday: 'Not yet available as a live lock. Self-destructs ciphertext and in-memory decryption context after first open.',
    plannedArch: 'Scheduled for a future release. Secrets, surprises, one-time credentials, and ephemeral confessions.'
  },
  {
    id: 'tap_unseal',
    num: '08',
    ...
    blockerType: 'soon',
    badgeClass: 'bg-emerald-950/60 text-emerald-400 border-emerald-800/60',
    reason: 'Deliberate gesture',
    desc: 'Requires deliberate tactile interaction, such as breaking a seal or tearing an envelope.',
    whyNotToday: 'Not yet available as a live lock. Requires deliberate, tactile interaction (breaking seal, tearing envelope).',
    plannedArch: 'Scheduled for a future release. Box chaining, surprise sequences, intentional consent & agreement moments.'
  },
  {
    id: 'one_time_magic_key',
    num: '10',
    ...
    blockerType: 'soon',
    badgeClass: 'bg-emerald-950/60 text-emerald-400 border-emerald-800/60',
    reason: 'Single-use fragment',
    desc: 'Uses a single-use asymmetric, ephemeral key fragment to unlock one target Box.',
    whyNotToday: 'Not yet available as a live lock. A single-use asymmetric ephemeral key fragment that unlocks one target Box.',
    plannedArch: 'Scheduled for a future release. Personalized VIP delivery without demanding recipient account registration.'
  },
  {
    id: 'totp',
    num: '11',
    ...
    blockerType: 'soon',
    badgeClass: 'bg-emerald-950/60 text-emerald-400 border-emerald-800/60',
    reason: 'Authenticator code',
    desc: 'Validates a six-digit rolling authenticator-app code based on RFC 6238.',
    whyNotToday: 'Not yet available as a live lock. Validates a 6-digit rolling authenticator app code (RFC 6238).',
    plannedArch: 'Scheduled for a future release. Sensitive recovery notes, crypto seeds, master passwords, ops playbooks.'
  },
  {
    id: 'signed_sender',
    num: '12',
    ...
    blockerType: 'soon',
    badgeClass: 'bg-emerald-950/60 text-emerald-400 border-emerald-800/60',
    reason: 'Ed25519 signature',
    desc: 'The recipient browser validates an Ed25519 creator signature before decryption.',
    whyNotToday: 'Not yet available as a live lock. Recipient browser validates Ed25519 creator signature before decrypting.',
    plannedArch: 'Scheduled for a future release. Legal notices, creator drops, authentic communications, leak-proof memos.'
  },
  {
    id: 'invite_code',
    num: '15',
    ...
    badgeClass: 'bg-emerald-950/60 text-emerald-400 border-emerald-800/60',
    reason: 'Gated tokens',
    desc: 'Gates access using reusable or quota-bounded invitation tokens.',
    whyNotToday: 'Not yet available as a live lock. Gated by reusable or quota-bounded invitation tokens.',
    plannedArch: 'Scheduled for a future release. Private alpha communities, secret societies, club access, gated launches.'
  },
  {
    id: 'two_person',
    num: '17',
    ...
    blockerType: 'soon',
    badgeClass: 'bg-emerald-950/60 text-emerald-400 border-emerald-800/60',
    reason: 'Shamir sharing',
    desc: 'Uses Shamir Secret Sharing so two independent keyholders must reconstruct the key.',
    whyNotToday: 'Not yet available as a live lock. Shamir Secret Sharing requires two independent keyholders to reconstruct key.',
    plannedArch: 'Scheduled for a future release. Escrow releases, shared family wealth vaults, dual-cofounder corporate actions.'
  },
  {
    id: 'location',
    num: '19',
    ...
    badgeClass: 'bg-emerald-950/60 text-emerald-400 border-emerald-800/60',
    reason: 'GPS radius',
    desc: 'Verifies that the recipient’s device is within a designated GPS radius.',
    whyNotToday: 'Not yet available as a live lock. Verifies recipient device geolocation within a designated GPS radius.',
    plannedArch: 'Scheduled for a future release. Physical scavenger hunts, museum tours, real-estate walkthroughs, geo-drops.'
  },
  {
    id: 'browser_key',
    num: '22',
    ...
    blockerType: 'soon',
    badgeClass: 'bg-emerald-950/60 text-emerald-400 border-emerald-800/60',
    reason: 'Seamless return visits',
    desc: 'Uses a locally stored IndexedDB private key to enable seamless unlocking on recurring visits.',
    whyNotToday: 'Not yet available as a live lock. Locally stored IndexedDB private key unlocks recurring visits seamlessly.',
    plannedArch: 'Scheduled for a future release. Frictionless private workspace access without conventional usernames or passwords.'
  },
  {
    id: 'puzzle',
    num: '23',
    ...
    blockerType: 'soon',
    badgeClass: 'bg-emerald-950/60 text-emerald-400 border-emerald-800/60',
    reason: 'Riddles & ciphers',
    desc: 'Unlocks after correctly solving an in-browser riddle, cipher, or mini-game.',
    whyNotToday: 'Not yet available as a live lock. Unlocks upon correctly solving an in-browser riddle, cipher, or mini-game.',
    plannedArch: 'Scheduled for a future release. ARGs, romantic Easter-egg messages, educational quests, hacker recruitment.'
  },
  {
    id: 'proof_of_human',
    num: '24',
    ...
    desc: 'Uses an interactive zero-knowledge puzzle or gesture challenge to stop automated scrapers and bots.',
    whyNotToday: 'Not yet available as a live lock. Interactive zero-knowledge puzzle or gesture challenge that stops bot scrapers.',
    plannedArch: 'Scheduled for a future release. Public links protected from automated web crawlers and AI web-scraping.'
  }
];
...
let state = {
  activeLocks: ['passcode', 'time_capsule'],
  solvedLocks: new Set(),
  subKeys: new Map(),
  threshold: 'all',
  mode: 'dashboard',
...
function renderRoadmapModal() {
  ...
  ROADMAP_LOCKS.forEach(lock => {
    const card = document.createElement('div');
    card.className = 'p-4 sm:p-5 rounded-2xl border transition bg-slate-950/80 border-slate-800/90 ' +
      (lock.status === 'COMING SOON' ? 'hover:border-emerald-500/40' : 'hover:border-amber-500/40');
    ...
    card.innerHTML = `
      ...
      <div class="space-y-2 text-xs">
        ${lock.status === 'COMING SOON' ? `
        <div class="bg-emerald-950/20 border border-emerald-900/30 rounded-xl p-2.5 sm:p-3 font-mono">
          <span class="text-emerald-400 font-semibold block mb-0.5">✨ COMING SOON</span>
          <span class="text-slate-300 leading-relaxed">${lock.desc}</span>
        </div>
        <div class="bg-slate-900/60 border border-slate-800/80 rounded-xl p-2.5 sm:p-3 font-mono">
          <span class="text-cyan-400 font-semibold block mb-0.5">🛠️ Planned behaviour:</span>
...
function initMarquees() {
  ...
  rails.forEach(rail => {
    ...
    rail.addEventListener('wheel', () => { hold(); release(1500); }, { passive: true });
    rail.addEventListener('touchstart', hold, { passive: true });
    rail.addEventListener('touchend', () => release(2400), { passive: true });
    rail.addEventListener('mouseenter', hold);
    rail.addEventListener('mouseleave', () => release(0));
    rail.addEventListener('focusin', hold);
    rail.addEventListener('focusout', () => release(0));
    rail.addEventListener('pointerup', () => release(1800));
    rail.addEventListener('pointercancel', () => release(1800));
    ...
    // Seamless infinite auto-marquee.
    // The card set is duplicated once (aria-hidden clones) and the rail then
    // drifts at a constant speed, wrapping at the halfway mark so the loop
    // never visibly jumps. Pauses on hover, drag, focus, and hidden tabs.
    // scroll-snap is switched off while drifting (and restored on interaction)
    // because x-proximity snapping quantizes small programmatic scroll steps.
    if (!reduceMotion) {
      const cloneCards = () => {
        if (rail.dataset.marqueeCloned === '1') return;
        ...
        originals.forEach(node => {
          ...
          copy.querySelectorAll('button, a, input, select, textarea, [tabindex]').forEach(el => {
            el.setAttribute('tabindex', '-1');
            el.setAttribute('aria-hidden', 'true');
          });
          rail.appendChild(copy);
        });
        rail.dataset.marqueeCloned = '1';
      };
      cloneCards();

      const SPEED = 26; // px per second — a calm drift, not a scroll hijack
      const drift = (ts) => {
        ...
        if (half <= rail.clientWidth + 8) { offset = rail.scrollLeft; return; }
        if (paused || dragging || document.hidden) {
          rail.classList.remove('autodrift');
          offset = rail.scrollLeft;
          return;
        }
        rail.classList.add('autodrift');
